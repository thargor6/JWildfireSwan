...
export class Framebuffers implements CloseableBuffers {
  ...
  constructor(private gl: WebGLRenderingContext, textures: Textures) {
        ...
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textures.texture0_array[0], 0)

        this.FBO1 = gl.createFramebuffer()!
        gl.bindFramebuffer(gl.FRAMEBUFFER, this.FBO1)
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textures.texture1_array[0], 0)

        this._FBO0 = gl.createFramebuffer()!
        gl.bindFramebuffer(gl.FRAMEBUFFER, this._FBO0)
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textures._texture0_array[0], 0)

        this._FBO1 = gl.createFramebuffer()!
        gl.bindFramebuffer(gl.FRAMEBUFFER, this._FBO1)
        gl.framebufferTexture2D(gl.FRAMEBUFFER, gl.COLOR_ATTACHMENT0, gl.TEXTURE_2D, textures._texture1_array[0], 0)
